...
const NodeProxyAPI = new API(`/api/proxy/node`, {
  'Content-Type': 'application/json',
})

const CallRPC = (method, params) => {
  if (process.browser) {
    return NodeProxyAPI.POST('', { method, jsonrpc: '2.0', id: 1, params })
  } else {
    return NodeAPI.POST('', { method, jsonrpc: '2.0', id: 1, params })
  }
}
...
const getRecentTransactions = ({ startVersion, limit, includeEvents }) =>
  CallRPC('get_recent_transactions', [startVersion, limit, includeEvents])
...
const getRecentAccountTransactions = ({ account, start, limit, includeEvents }) =>
  CallRPC('get_recent_account_transactions', [account, start, limit, includeEvents])
